...
## [1.64.6] — 2026-08-16 (plain-language product explanation)

This documentation patch states the product category directly: the AI supplies
replaceable reasoning, while the user-owned folder supplies durable knowledge,
rules, procedures, provenance, and recovery history. The README, Product
Overview, and How the System Works now explain the problem solved, the five
coordinated jobs, the intended user, the tradeoff of visible structure, and the
fact that direct-file compliance controls are not a security sandbox. Product
behavior is unchanged. The immutable v1.64.5 release remains preserved and
superseded.

## [1.64.5] — 2026-08-16 (temporary-repository checkpoint identity)

This patch release corrects the remaining public CI setup defect found after
v1.64.4 was tagged. The workflow now configures its neutral Git identity at
runner scope, so safety checkpoints also work inside the disposable Git
repositories created by the test suite. The product's checkpoint refusal was
correct and prevented mutation when identity was absent; only the CI setup was
incomplete. A regression now requires global ephemeral-runner identity. The
immutable v1.64.2, v1.64.3, and v1.64.4 tags remain unchanged and superseded.

## [1.64.4] — 2026-08-16 (self-contained public CI)

This patch release separates two evidence scopes that v1.64.3 incorrectly ran
as one job. Public GitHub CI now proves the clean distribution using only its
own files, rebuilds disposable generated indexes, configures checkpoint Git
identity, and reports an exact bounded skip set for tests that require the
separately pinned historical authority. Release qualification still runs every
historical migration, fault, convergence, and Git/no-Git recovery test against
that authority; none is waived or represented as public-checkout evidence.
GitHub Actions are pinned by immutable commit, Linux matrix failures no longer
cancel sibling coverage, and release tags still require the macOS job. The
immutable v1.64.2 and v1.64.3 releases are preserved and superseded.
