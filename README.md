...
Personal AI OS is the **user-owned continuity layer between you and a filesystem-capable AI**. The AI supplies replaceable reasoning; this folder supplies the durable memory, operating rules, procedures, provenance, and recovery history. It is not another model, chatbot, cloud account, or proprietary memory service.

In practical terms, it turns an ordinary folder into a governed working environment:

- **Your knowledge stays yours:** notes, sources, decisions, preferences, and methods remain readable Markdown and YAML.
- **A new AI does not start from zero:** it follows one entry chain, learns the rules for the current task, and loads only the context it needs.
- **Retrieval is selective and inspectable:** built-in search applies domain, status, and privacy eligibility before relevance, then names the records used.
- **Writes are proportional to risk:** ordinary notes stay lightweight; consequential and system changes add dependency review, checkpoints, validation, and recovery.
- **The system can evolve without abandoning old copies:** versioned migrations and tested rollback paths carry supported installations forward.

This is most useful for people doing long-lived work with AI who want continuity across sessions and vendors without giving up inspectability or ownership. The tradeoff is deliberate structure: it asks both the user and the AI to follow visible procedures instead of relying on invisible product memory or unconstrained agent behavior.
