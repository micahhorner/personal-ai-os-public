...
## 0. The two halves of the system

Personal AI OS deliberately separates the part you rent from the part you own:

- **The AI supplies cognition:** it interprets a request, reasons about the material, drafts, and uses tools. Claude Code and Codex are the shipped connections today; another compatible runtime can replace them.
- **The folder supplies continuity:** it holds the knowledge, preferences, rules, task procedures, provenance, validation machinery, and recovery history that should outlive any one AI session or vendor.

The system works when those halves meet. The AI is not being trained or given magical permanent memory. At the beginning of a task it reads a small, deterministic entry chain from the folder. That chain tells it which rules always apply, what kind of job this is, and which step-by-step procedure and records to load. Deterministic scripts handle checks, indexes, snapshots, migrations, and other work that should not depend on model judgment.

This solves a specific problem: useful context normally accumulates inside chats and vendor products, while direct-file agents can act inconsistently or too broadly. Here, context accumulates in portable files and supported actions pass through visible controls. The result is not autonomous perfection or a security sandbox; it is a governed, inspectable way for replaceable AI runtimes to work on a durable body of user-owned knowledge.
