...
> **Current publication release.** Version 1.64.3 corrects the public CI workflow while retaining the documented product behavior. Before upgrading an irreplaceable or only copy, verify that the `v1.64.3` tag resolves to the release commit and that its separately published, target-bound qualification evidence passes. Earlier release statements remain part of the upgrade record.
