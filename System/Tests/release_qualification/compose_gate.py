...
def compose(*, repository: str, matrix_path: str, qualification_path: str,
            ...
            predecessor_commit: str | None = None) -> dict[str, Any]:
    ...
    if predecessor and not all(isinstance(value, str) and value for value in (
            predecessor_repository, predecessor_ref, predecessor_commit)):
        raise ComposeError(
            "predecessor authority requires repository, ref, and full commit pin")
    if predecessor and (len(predecessor_commit) != 40 or any(
            char not in "0123456789abcdef" for char in predecessor_commit)):
        raise ComposeError("predecessor commit must be a full lowercase SHA-1")
    ...
    if split:
        ...
        if predecessor:
            predecessor_record = estate.load_release(
                predecessor_repository, predecessor_ref)
            if predecessor_record.commit != predecessor_commit:
                raise ComposeError("predecessor ref does not match its pinned commit")
            if not (estate._version_key(authority_tip.system_version)
                    < estate._version_key(predecessor_record.system_version)
                    < estate._version_key(matrix.target.version)):
                raise ComposeError(
                    "predecessor version must follow historical authority and precede target")
        ...
        if predecessor_record is not None:
            records.append(predecessor_record)
        ...
        if predecessor_record is not None:
            authority["predecessor_commit"] = predecessor_record.commit
            authority["predecessor_version"] = predecessor_record.system_version
    else:
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--predecessor-ref")
