...
class TestPublicCIContract(unittest.TestCase):
    ...
    def test_checkpoint_identity_and_public_scope_are_explicit(self):
        ...
        self.assertEqual(workflow.count("git config --global user.name"), 2)
        self.assertEqual(workflow.count("git config --global user.email"), 2)
        self.assertNotIn("git config user.name", workflow)
        self.assertNotIn("git config user.email", workflow)
