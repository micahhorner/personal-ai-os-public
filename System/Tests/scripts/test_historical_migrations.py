...
class TestHistoricalMigrationEvidence(unittest.TestCase):
    ...
    def test_bridge_and_final_release_ref_status_are_explicit(self):
        ...
        self.assertEqual(
            unresolved["target_release_ref"], "refs/tags/v1.64.3"
        )
        self.assertEqual(
    ...
    def test_direct_estate_has_22_supported_sources_to_1643(self):
        available = discover(VAULT / "System/Migrations")
        direct = [
            item for item in available
        ...
        self.assertEqual(len(generated_direct), 22)
        self.assertEqual(len({item.from_version for item in direct}), 22)
    ...
    def test_022_identity_exact_no_write_and_custom_license_refusal(self):
        source = self.catalog["official_sources"][-2]
        ...
        self.assertEqual(manifest.to_version, "1.64.3")
