...
class TestHistoricalMigrationEvidence(unittest.TestCase):
    ...
    def test_official_and_refused_identity_coverage_is_exact_and_unique(self):
        ...
        self.assertEqual(len(official), 25)
        ...
        self.assertEqual(
            ...
            [
                ...
                "1.64.1", "1.64.2", "1.64.3", "1.64.4", "1.64.5",
            ],
        )
    ...
    def test_generated_files_and_manifest_hashes_are_current(self):
        ...
        manifests.append(
            VAULT / "System/Migrations/023-official-1-64-2/migration.yaml"
        )
        manifests.append(
            VAULT / "System/Migrations/024-official-1-64-3/migration.yaml"
        )
        manifests.append(
            VAULT / "System/Migrations/025-official-1-64-4/migration.yaml"
        )
        manifests.append(
            VAULT / "System/Migrations/026-official-1-64-5/migration.yaml"
        )
    ...
    def test_every_old_and_payload_hash_matches_exact_git_evidence(self):
        ...
        for index, source in enumerate(self.catalog["official_sources"], 1):
            ...
            modern_edges = {
                "1.64.1": "022-official-1-64-1",
                "1.64.2": "023-official-1-64-2",
                "1.64.3": "024-official-1-64-3",
                "1.64.4": "025-official-1-64-4",
                "1.64.5": "026-official-1-64-5",
            }
            name = (
                "021-official-1-64-0"
                if source.get("transform") == "postcondition-only"
                else modern_edges[version]
                ...
                for effect in manifest.data["transform"]["replace_exact_files"]:
                    ...
                    if name == "021-official-1-64-0":
                        destination.write_bytes(generator.git_bytes(
                            next(item["commit"] for item in self.catalog["official_sources"]
                                 if str(item["version"]) == "1.64.0"),
                            effect["path"],
                        ))
                    else:
                        destination.write_bytes(
    ...
    def test_official_164_custom_root_bytes_refuse_before_replacement(self):
        source = next(
            item for item in self.catalog["official_sources"]
            if str(item["version"]) == "1.64.0"
        )
    ...
    def test_bridge_and_final_release_ref_status_are_explicit(self):
        bridge = next(item for item in self.catalog["official_sources"]
                      if str(item["version"]) == "1.64.0")
        ...
        self.assertEqual(
            unresolved["target_release_ref"], "refs/tags/v1.64.6"
        )
        self.assertEqual(
    ...
    def test_direct_estate_has_25_supported_sources_to_1646(self):
        available = discover(VAULT / "System/Migrations")
        direct = [
            item for item in available
        ...
        self.assertEqual(len(generated_direct), 25)
        self.assertEqual(len({item.from_version for item in direct}), 25)
    ...
    def test_022_identity_exact_no_write_and_custom_license_refusal(self):
        ...
        self.assertEqual(manifest.to_version, "1.64.6")
    ...
    def test_023_identity_exact_no_write_and_custom_license_refusal(self):
        source = next(
            item for item in self.catalog["official_sources"]
            if str(item["version"]) == "1.64.2"
        )
        self.assertEqual(str(source["version"]), "1.64.2")
        self.assertEqual(
            source["source_identity"],
            "070ad9ddad973dde697be01eda81f7f6865d6ff7a30d6806cc012987abbcf65c",
        )
        source_dir = VAULT / "System/Migrations/023-official-1-64-2"
        manifest = load_manifest(source_dir / "migration.yaml")
        self.assertEqual(manifest.from_version, "1.64.2")
        self.assertEqual(manifest.to_version, "1.64.6")
        self.assertEqual(manifest.source_identity, source["source_identity"])
        effect = manifest.data["transform"]["replace_exact_files"]
        self.assertEqual(len(effect), 1)
        ...
        with tempfile.TemporaryDirectory() as temp:
            root = Path(temp) / "vault"
            shutil.copytree(source_dir, root / "System/Migrations/023-official-1-64-2")
            license_path = root / "LICENSE.md"
            license_path.parent.mkdir(parents=True, exist_ok=True)
            license_path.write_bytes((source_dir / effect["payload"]).read_bytes())
            contract = migrate_mod.exact_transform_contract(str(root), manifest)
            ...
            license_path.write_text("custom license\n")
            before = license_path.read_bytes()
            with self.assertRaisesRegex(MigrationError, "old_sha256 precondition mismatch"):
                migrate_mod._replace_exact_files(str(root), manifest, apply=True)
            self.assertEqual(license_path.read_bytes(), before)

    def test_024_identity_exact_no_write_and_custom_license_refusal(self):
        source = next(
            item for item in self.catalog["official_sources"]
            if str(item["version"]) == "1.64.3"
        )
        self.assertEqual(str(source["version"]), "1.64.3")
        self.assertEqual(
            source["source_identity"],
            "eda52f8514060ffaf128407ae142574ab21d52ef667806db4cf48c8873d71ae7",
        )
        source_dir = VAULT / "System/Migrations/024-official-1-64-3"
        manifest = load_manifest(source_dir / "migration.yaml")
        self.assertEqual(manifest.from_version, "1.64.3")
        self.assertEqual(manifest.to_version, "1.64.6")
        self.assertEqual(manifest.source_identity, source["source_identity"])
        effect = manifest.data["transform"]["replace_exact_files"]
        self.assertEqual(len(effect), 1)
        self.assertEqual(effect[0]["path"], "LICENSE.md")
        self.assertEqual(effect[0]["old_sha256"], effect[0]["new_sha256"])
        with tempfile.TemporaryDirectory() as temp:
            root = Path(temp) / "vault"
            shutil.copytree(source_dir, root / "System/Migrations/024-official-1-64-3")
            license_path = root / "LICENSE.md"
            license_path.parent.mkdir(parents=True, exist_ok=True)
            license_path.write_bytes((source_dir / effect[0]["payload"]).read_bytes())
            contract = migrate_mod.exact_transform_contract(str(root), manifest)
            self.assertFalse(contract[0]["needs_write"])
            license_path.write_text("custom license\n")
            before = license_path.read_bytes()
    ...
    def test_025_identity_exact_no_write_and_custom_license_refusal(self):
        source = next(
            item for item in self.catalog["official_sources"]
            if str(item["version"]) == "1.64.4"
        )
        self.assertEqual(
            source["source_identity"],
            "5b7c466eadeb65ce772818a73d2e6bb41598a488639a0a1433eda7536b1977b6",
        )
        source_dir = VAULT / "System/Migrations/025-official-1-64-4"
        manifest = load_manifest(source_dir / "migration.yaml")
        self.assertEqual(manifest.from_version, "1.64.4")
        self.assertEqual(manifest.to_version, "1.64.6")
        self.assertEqual(manifest.source_identity, source["source_identity"])
        effect = manifest.data["transform"]["replace_exact_files"]
        self.assertEqual(len(effect), 1)
        ...
        with tempfile.TemporaryDirectory() as temp:
            root = Path(temp) / "vault"
            shutil.copytree(source_dir, root / "System/Migrations/025-official-1-64-4")
            license_path = root / "LICENSE.md"
            license_path.parent.mkdir(parents=True, exist_ok=True)
            license_path.write_bytes((source_dir / effect[0]["payload"]).read_bytes())
            contract = migrate_mod.exact_transform_contract(str(root), manifest)
